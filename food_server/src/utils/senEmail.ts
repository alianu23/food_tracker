...
export const sendEmail = async ({ email, otp, token }: IEmailProps) => {
  const htmlTemplate = otp
    ? generateOtp(otp, email)
    : generateLink(token as string, email);
...
const generateOtp = (otp: string, email: string) => {
  return `
    <div style="min-width:1000px; overflow:auto; line-height:2">
      <div style="margin:50px auto; width:70%; padding:20px 0">
        ...
        <p style="font-size:1.1em">Hello Dear, ${email}</p>
        <p>
...
const generateLink = (token: string, email: string) => {
  return `
    <div style="min-width:1000px; overflow:auto; line-height:2">
      <div style="margin:50px auto; width:70%; padding:20px 0">
        <div style="border-bottom:1px solid #eee">
